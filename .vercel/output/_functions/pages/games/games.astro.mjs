...
import 'kleur/colors';
import { g as getCollection, $ as $$Layout, F as FechaRelativa } from '../../chunks/FechaRelativa_CAYqGtjl.mjs';
import { $ as $$Coment, a as $$Discord, b as $$XTwiter } from '../../chunks/XTwiter_1pSEv-Hi.mjs';
import { $ as $$Header } from '../../chunks/header_BP1m_dWT.mjs';
export { renderers } from '../../renderers.mjs';

const $$Games = createComponent(async ($$result, $$props, $$slots) => {
  const articles = (await getCollection("articles")).filter((article) => article.data.categoryName === "games").sort((a, b) => {
  ...
  const title = "Videojuegos";
  return renderTemplate`${renderComponent($$result, "Layout", $$Layout, { "title": title }, { "default": async ($$result2) => renderTemplate` ${renderComponent($$result2, "Header", $$Header, {})} ${maybeRenderHead()}<main class="flex flex-col lg:flex-row justify-between gap-10 max-w-[1350px] m-auto"> <section> ${articles.map((article) => {
